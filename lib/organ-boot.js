...
import { createToolFallbackHandler } from './tool-fallback-handler.js';
 ...
 *                                       (optional). MP-TOOL-1 (D1): per-organ override for the
 *                                       tool-call surface. Receives the full tool_call_request OTM
 *                                       envelope, returns a tool_call_response payload (use helpers
 *                                       from `@coretex/organ-boot/tool-errors`). If omitted, the
 *                                       factory installs a NOT_IMPLEMENTED fallback so every organ
 *                                       responds well-formed regardless of per-organ wiring status.
...
export async function createOrgan(config) {
  ...
  // MP-TOOL-1 D1: factory supplies NOT_IMPLEMENTED fallback by default; organs
  // that register their own toolCallHandler override via config. Override
  // semantics are a pure shadow — the supplied handler fully replaces the
  // fallback (no chaining).
  const effectiveToolCallHandler = toolCallHandler || createToolFallbackHandler(name);

  loop = createLiveLoop({
    spine,
    onMessage,
    onBroadcast,
    toolCallHandler: effectiveToolCallHandler,
    drainInterval,
    drainLimit: 10,
  });
