  ...
  async function chat(messages, options = {}) {
    ...
    try {
      ...
      if (defaultProvider === 'anthropic') {
        ...
        result = await callOpenAICompatible(apiKey, model, messages, tokens, options);
      } else {
...
async function callOpenAICompatible(apiKey, model, messages, maxTokens, options) {
  const url = (options.baseUrl || 'http://127.0.0.1:11434') + '/v1/chat/completions';
  ...
  // Some OpenAI-compatible APIs require auth
  ...
  if (!res.ok) {
    const errorBody = await res.text();
    throw new Error(`OpenAI-compatible API ${res.status}: ${errorBody}`);
  }
