 ...
 *
 * Binding:
 *   entity:<tenant_urn> ─relation:llm-usage─→ llm-usage-event:<urn>
 *
  ...
  function writeLLMUsageEvent(event) {
    ...
    Promise.resolve()
      ...
      .then(() => {
        ...
        // (if any) see the source first.
        const ubn = `ubn:llm-ops:llm-usage:${iso.replace(/[:.]/g, '-')}-${suffix}`;
...
export { buildModelUrn, buildAgentUrn, buildOrganUrn };
