...
export function createSpineClient(opts = {}) {
  ...
  /**
   * Send a directed/broadcast envelope via POST /messages WITHOUT throwing on
   * HTTP 4xx/5xx. Returns a structured result so callers can react to routing
   * failures without catching exceptions:
   *
   *   { ok: true,  status, data }                       // on 2xx
   *   { ok: false, status, error, message, data }      // on non-2xx
   *   { ok: false, status: 0, error: 'NETWORK_ERROR', message }  // on network failure
   *
   * Opt-in alternative to send(). Callers that want throw semantics keep using
   * send(). Callers that want structured errors (e.g. MCP-Router's call-translator,
   * which must not propagate unhandled rejections) use sendSafe().
   */
  async function sendSafe(envelope) {
